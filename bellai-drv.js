      ...
      async function sendAnalyticsData() {
        try {
          ...
          const analyticsData = {
            ...
            // Chat widget specific data
            chatWidgetVersion: "1.9.2",
            chatWidgetInitialized: true,
            userManuallyClosedChat: userManuallyClosedChat
          };

          // Send analytics data to Drivonic endpoint
          const response = await fetch('https://us1-audiencesync.drivonic.com/api/intndr/track', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Access-Control-Allow-Origin': '*',
          ...
          if (response.ok) {
            console.log('[BellaAI] Analytics data sent successfully');
          } else {
            console.warn('[BellaAI] Failed to send analytics data:', response.status);
          }
        } catch (error) {
          console.error('[BellaAI] Error sending analytics data:', error);
        }
      }

      // Send analytics data on page load
      sendAnalyticsData();

      // Also send analytics data when URL changes (for SPA navigation)
      let lastAnalyticsUrl = window.location.href;
      const analyticsObserver = new MutationObserver(() => {
        const currentUrl = window.location.href;
        if (currentUrl !== lastAnalyticsUrl) {
          lastAnalyticsUrl = currentUrl;
          // Debounce analytics sending to avoid too many requests
          setTimeout(() => {
            sendAnalyticsData();
          }, 1000);
        }
      });
      analyticsObserver.observe(document, { subtree: true, childList: true });
