window.BellaAIVoiceWS=null,window.BellaAIVoiceActive=!1,window.BellaAISeenAudio=window.BellaAISeenAudio||new Set,window.BellaAIEarlyAudioBuffer=[],window.BellaAISpeakingFrames=0,window.BellaAIVoiceChatAudio={q:[],audioCtx:null,playing:!1,currentSrc:null,currentGain:null,paused:!1,agentSpeaking:!1,_resumeTimeout:null,_lastB64:null,enqueue(e,t){t?(this.audioCtx=t,this.q.push(e),console.log("[BellaAI][AUDIO] Enqueued audio chunk. Queue length:",this.q.length),this.playing||this.paused||this._playNext()):console.warn("[BellaAI][AUDIO] enqueue: No audio context provided")},_playNext(){var e;this.paused?console.log("[BellaAI][AUDIO] _playNext: Paused, not playing next chunk"):window.BellaAITranscriptTracking.abortCurrentResponse?(console.log("[BellaAI][AUDIO] _playNext: Aborting due to interrupt flag"),this.stop()):(e=this.q.shift())?(this.playing=!0,console.log("[BellaAI][AUDIO] _playNext: Decoding and playing next chunk"),this._decodeAndBuffer(e,this.audioCtx).then(e=>{if(this.audioCtx&&"closed"!==this.audioCtx.state){if(this.currentSrc){try{this.currentSrc.stop(0)}catch(e){}try{this.currentSrc.disconnect()}catch(e){}this.currentSrc=null}var t=this.audioCtx.createBufferSource(),o=(t.buffer=e,this.audioCtx.createGain());o.gain.value=1,t.connect(o),o.connect(this.audioCtx.destination),this.currentSrc=t,this.agentSpeaking=!0,console.log("[BellaAI][AUDIO] Starting audio playback, buffer length:",e.length),t.onended=()=>{console.log("[BellaAI][AUDIO] Audio playback ended"),this.agentSpeaking=!1,this.currentSrc=null,this._playNext()};try{t.start(0),console.log("[BellaAI][AUDIO] Audio playback started")}catch(e){console.error("[BellaAI][AUDIO] Error starting audio source:",e),this.currentSrc=null,this.agentSpeaking=!1,this._playNext()}}else console.error("[BellaAI][AUDIO] AudioContext is closed, cannot play audio"),this.playing=!1}).catch(e=>{console.error("[BellaAI][AUDIO] Error decoding/playing audio:",e),this.currentSrc=null,this._playNext()})):(this.playing=!1,this.currentSrc=null,this.agentSpeaking=!1,console.log("[BellaAI][AUDIO] _playNext: Queue empty, agent done speaking"))},_decodeAndBuffer(s,d){return this._lastB64=s,console.log("[BellaAI] Decoding audio chunk, length:",s.length),new Promise((e,t)=>{try{if(!s||s.length<100)console.error("[BellaAI] Invalid base64 data in _decodeAndBuffer:",s.length),t(new Error("Invalid base64 data"));else if(d&&"closed"!==d.state){var o=atob(s),a=(console.log("[BellaAI] Decoded binary length:",o.length),new ArrayBuffer(o.length)),n=new Uint8Array(a);for(let e=0;e<o.length;e++)n[e]=o.charCodeAt(e);var i=new Int16Array(Math.floor(n.length/2));for(let e=0,t=0;e<n.length-1;e+=2,t++)i[t]=255&n[e]|(255&n[e+1])<<8;console.log("[BellaAI] Created Int16Array with",i.length,"samples");var r=d.createBuffer(1,i.length,16e3),l=r.getChannelData(0);for(let e=0;e<i.length;e++)l[e]=i[e]/32768;console.log("[BellaAI] Successfully created audio buffer"),e(r)}else console.error("[BellaAI] Invalid or closed audio context"),t(new Error("Invalid audio context"))}catch(e){console.error("[BellaAI] Error in _decodeAndBuffer:",e),t(e)}})},pause(){if(this.paused)console.log("[BellaAI][AUDIO] pause: Already paused");else if(this.paused=!0,console.log("[BellaAI][AUDIO] pause: Pausing playback"),this.currentSrc){try{this.currentSrc.stop(0)}catch(e){}this.currentSrc.disconnect(),this.currentSrc.buffer&&this._lastB64&&(this.q.unshift(this._lastB64),console.log("[BellaAI][AUDIO] pause: Pushed last chunk back to queue")),this.currentSrc=null}},resume(){this.paused?(this.paused=!1,console.log("[BellaAI][AUDIO] resume: Resuming playback"),this._playNext()):console.log("[BellaAI][AUDIO] resume: Not paused")},stop(){if(this.currentSrc){try{this.currentSrc.stop(0)}catch(e){}this.currentSrc.disconnect(),this.currentSrc=null,console.log("[BellaAI][AUDIO] stop: Stopped current source")}this.q=[],this.playing=!1,this.paused=!1,this.agentSpeaking=!1,this.audioCtx&&"running"===this.audioCtx.state&&(this.audioCtx.suspend().catch(()=>{}),console.log("[BellaAI][AUDIO] stop: Suspended audio context")),this.audioCtx=null,console.log("[BellaAI][AUDIO] stop: Cleared queue and reset state")}},window.BellaAITranscriptTracking={lastUserTranscriptId:null,lastAgentResponseId:null,processedResponses:new Set,currentConversationId:null,voiceModeAnnounced:!1,defaultGreetingShown:!1,lastResponseText:null,lastAudioId:null,abortCurrentResponse:!1,reset:function(){this.lastUserTranscriptId=null,this.lastAgentResponseId=null,this.processedResponses.clear(),this.currentConversationId=null,this.voiceModeAnnounced=!1,this.defaultGreetingShown=!1,this.lastResponseText=null,this.lastAudioId=null,this.abortCurrentResponse=!1},isResponseProcessed:function(e){return!!e&&(this.processedResponses.has(e)||this.lastAgentResponseId===e)},markResponseProcessed:function(e){e&&(this.lastAgentResponseId=e,this.processedResponses.add(e))}},(()=>{let t=()=>{try{let e=new(window.AudioContext||window.webkitAudioContext);e.resume().then(()=>e.close()).catch(()=>{}),window.BellaAIVoiceChatAudio&&window.BellaAIVoiceChatAudio.audioCtx&&window.BellaAIVoiceChatAudio.audioCtx.resume().catch(()=>{})}catch(e){console.log("[BellaAI] Audio context unlock attempt error (non-critical):",e)}};["touchend","touchstart","click","keydown"].forEach(e=>window.addEventListener(e,t,{once:!0,passive:!0}))})(),(()=>{console.log("ChatVersion:","2.0");let g="";(async()=>{try{var e=await(await fetch("https://api.ipify.org?format=json")).json();g=e.ip,console.log("User IP collected for chat")}catch(e){console.error("Error fetching IP:",e),g="unknown"}})();var e={webhook:{url:"",route:""},branding:{logo:"https://cdn-icons-png.flaticon.com/512/5962/5962463.png",name:"Bella : Product Specialist",welcomeText:""},style:{primaryColor:"#003f72",secondaryColor:"#003f72",position:"right",backgroundColor:"#ffffff",fontColor:"#333333"},dealer:{name:"",phone:"",website:"",searchPage:"",provider:""},overtake:!1};let x=window.ChatWidgetConfig?{webhook:{...e.webhook,...window.ChatWidgetConfig.webhook},branding:{...e.branding,...window.ChatWidgetConfig.branding},style:{...e.style,...window.ChatWidgetConfig.style},dealer:{...e.dealer,...window.ChatWidgetConfig.dealer},overtake:window.ChatWidgetConfig.overtake||!1,overtakePath:window.ChatWidgetConfig.overtakePath||"/"}:e;if(window.BellaAIChatWidgetInitialized)return;let t=!(window.BellaAIChatWidgetInitialized=!0);function o(e){try{var t={manuallyClosed:e,timestamp:(new Date).getTime()};localStorage.setItem("bellaaiChatState",JSON.stringify(t))}catch(e){console.error("Error saving chat state:",e)}}function a(){return"undefined"!=typeof crypto&&"function"==typeof crypto.randomUUID?crypto.randomUUID():"xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g,function(e){var t=16*Math.random()|0;return("x"===e?t:3&t|8).toString(16)})}function u(e){var t;return e?(t=/<[a-z][\s\S]*>/i.test(e),e=e.replace(/\[([^\]]+)\]\(([^)]+)\)/g,'<a href="$2" target="_blank">$1</a>'),(e=(e=(e=t?e:e.replace(/(https?:\/\/[^\s<]+)(?![^<]*>)/g,'<a href="$1" target="_blank">Visit Website</a>')).replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>")).replace(/\*([^*]+)\*/g,"<em>$1</em>")).replace(/\n/g,"<br>")):""}function m(){var e=document.getElementById("thinking-animation");e&&e.remove()}window.initialUtmParameters=(e=>{let o={};try{var a=new URL(e);let t=new URLSearchParams(a.search);var n,i,r=["utm_source","utm_medium","utm_campaign","utm_term","utm_content"];r.forEach(function(e){t.has(e)&&(o[e]=t.get(e))});for([n,i]of t.entries())n.startsWith("utm_")&&!r.includes(n)&&(o[n]=i)}catch(e){console.error("Error extracting UTM parameters:",e)}return o})(window.location.href),console.log("Initial UTM Parameters:",window.initialUtmParameters);let y="";function b(){if(y)try{var e={sessionId:y,messages:Array.from(k.children).filter(function(e){return e.classList.contains("chat-message")}).map(function(e){return{type:e.classList.contains("user")?"user":"bot",content:e.classList.contains("user")?e.textContent:e.innerHTML}}),inactivityMessageSent:w,promptBubbleShown:r,timestamp:(new Date).getTime(),utmParameters:window.initialUtmParameters||{}};localStorage.setItem("bellaaiChatSession",JSON.stringify(e))}catch(e){console.error("Error saving chat session:",e)}}function n(){try{var e=localStorage.getItem("bellaaiChatSession");if(e){var t=JSON.parse(e),o=(new Date).getTime()-t.timestamp;if(o<864e5)return k.innerHTML="",y=t.sessionId,t.inactivityMessageSent&&(w=!0),t.promptBubbleShown&&(r=!0),t.utmParameters&&(window.initialUtmParameters=t.utmParameters),t.messages&&0<t.messages.length&&t.messages.forEach(function(e){var t=document.createElement("div");t.className="chat-message "+e.type,"user"===e.type?t.textContent=e.content:t.innerHTML=e.content,k.appendChild(t)}),!0;localStorage.removeItem("bellaaiChatSession")}}catch(e){console.error("Error loading saved chat session:",e),localStorage.removeItem("bellaaiChatSession")}return!1}let f,R=12e4,w=!1;function v(){clearTimeout(f),w||(f=setTimeout(function(){var e;d.classList.contains("open")&&y&&((e=document.createElement("div")).className="chat-message bot",e.innerHTML=u("Do you have any questions I can help with?"),k.appendChild(e),k.scrollTop=k.scrollHeight,w=!0,b())},R))}let i,r=!1;function H(){var e=document.getElementById("prompt-bubble");e&&e.remove()}var l,e=`
    ...
    @media screen and (max-width: 600px) {
      ...
      `,e.id="thinking-animation",k.appendChild(e),k.scrollTop=k.scrollHeight})();try{var a=S||await T(),n={chatInput:e,sessionId:y,metadata:{dealer:x.dealer,pageContext:a,utmParameters:window.initialUtmParameters||{},userIP:g}},i=await fetch(x.webhook.url,{method:"POST",headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*","Access-Control-Allow-Methods":"POST, OPTIONS","Access-Control-Allow-Headers":"Content-Type"},body:JSON.stringify(n)});if(!i.ok)throw new Error("Failed to send message");let o="";var r=await i.text();try{var l=JSON.parse(r);Array.isArray(l)&&0<l.length&&l[0].output?o=l[0].output:l&&"object"==typeof l?o=l.output||l.response||l.message||"":"string"==typeof l&&(o=l)}catch(e){o=r}o&&""!==o.trim()||(o="I received your message, but I'm having trouble generating a response."),m();var s=o.match(/\[REDIRECT\](.*?)\[\/REDIRECT\]/);if(s&&s[1]){let t=(t=>{try{var e=new URL(t),o=new URLSearchParams(e.search);return o.set("utm_source","BellaAI"),o.set("utm_medium","chat"),o.set("utm_campaign","Drivonic-MarketBuilder-AIChat"),e.search=o.toString(),e.toString()}catch(e){return console.error("Error appending UTM parameters to URL:",e),t}})(s[1].trim());var d,c=o.replace(/\[REDIRECT\][\s\S]*?\[\/REDIRECT\]/g,"").trim();c&&((d=document.createElement("div")).className="chat-message bot",d.innerHTML=u(c),k.appendChild(d),k.scrollTop=k.scrollHeight),void setTimeout(()=>{var e=document.createElement("div");e.className="chat-message bot",e.innerHTML=u("Redirecting you now..."),k.appendChild(e),k.scrollTop=k.scrollHeight,b(),setTimeout(()=>{window.location.href=t},3e3)},1e3)}else{var p,h=o.replace(/\[REDIRECT\][\s\S]*?\[\/REDIRECT\]/g,"").trim();h&&((p=document.createElement("div")).className="chat-message bot",p.innerHTML=u(h),k.appendChild(p),k.scrollTop=k.scrollHeight),b(),w=!1,clearTimeout(f),v()}}catch(e){console.error("Error sending message:",e),m();t=document.createElement("div");t.className="chat-message bot",t.innerHTML=u("I apologize, but I'm having trouble connecting right now. Please try again in a moment."),k.appendChild(t),k.scrollTop=k.scrollHeight}}})(e),t.value="",t.style.height="auto")):console.error("[BellaAI] No textarea found for message sending")}c.addEventListener("click",function(){d.classList.contains("open")||(d.classList.add("open"),c.classList.add("hidden"),window.innerWidth<=600&&(document.body.style.overflow="hidden"),k.innerHTML="",n()||(w=!1,y=a(),T().then(()=>{z()})),b(),k.scrollTop=k.scrollHeight,v(),clearTimeout(i))}),C.addEventListener("click",function(){(N?O:D)(),o(t=!0),window.innerWidth<=600&&(document.body.style.overflow="")}),h.addEventListener("input",function(){this.style.height="auto",this.style.height=this.scrollHeight+"px"}),I.addEventListener("click",A),h.addEventListener("keypress",function(e){"Enter"!==e.key||e.shiftKey||(e.preventDefault(),A())}),window.addEventListener("resize",function(){window.innerWidth<=600?d.classList.contains("open")&&(document.body.style.overflow="hidden"):document.body.style.overflow=""});let S=null,E=null;async function T(){try{var e=document.querySelector("main, #main, .main-content, .content");if(e){var t=(()=>{if(e=localStorage.getItem("bellai_nav_links"))return JSON.parse(e);let o=new Set;function t(e){e&&e.querySelectorAll("a[href]").forEach(e=>{var t=e.getAttribute("href"),e=e.textContent.trim();t&&!t.startsWith("#")&&!t.startsWith("javascript:")&&e&&(t=new URL(t,window.location.origin).href,o.add(JSON.stringify({text:e,url:t})))})}var e=document.querySelector("nav"),a=document.querySelector("header"),e=(e&&t(e),a&&t(a),0===o.size&&document.querySelectorAll('[class*="nav"]').forEach(e=>t(e)),Array.from(o).map(e=>JSON.parse(e)));return localStorage.setItem("bellai_nav_links",JSON.stringify(e)),e})(),o=(y=window.location.pathname.toLowerCase()).includes("/new-vehicles/")||y.includes("/used-vehicles/")?"inventory":y.includes("/finance")?"finance":y.includes("/service")?"service":y.includes("/parts")?"parts":y.includes("/about")?"about":y.includes("/contact")?"contact":"/"===y||"/index.html"===y?"home":"other",a=(e=>{if(!e)return"";for(var t=(e=e.cloneNode(!0)).getElementsByTagName("script");0<t.length;)t[0].parentNode.removeChild(t[0]);for(var o=e.getElementsByTagName("style");0<o.length;)o[0].parentNode.removeChild(o[0]);let a=e.textContent||e.innerText;return a=(a=a.replace(/\s+/g," ").trim()).replace(/<[^>]*>/g,"")})(e);if(a){u=a,m=(m=new TextEncoder).encode(u),u=await crypto.subtle.digest("SHA-256",m);var n=await(m=Array.from(new Uint8Array(u))).map(e=>e.toString(16).padStart(2,"0")).join(""),i=((e,t)=>{try{var o=localStorage.getItem("bellaaiPageSummary");if(o){var{url:a,hash:n,summary:i,timestamp:r}=JSON.parse(o);if(a===e&&n===t&&Date.now()-r<36e5)return i}}catch(e){console.error("Error reading cached summary:",e)}return null})(window.location.href,n);if(i)return console.log("Using cached page summary"),{...i,navigationLinks:t};var r={title:document.title,url:window.location.href,description:document.querySelector('meta[name="description"]')?.getAttribute("content")||"",type:o,dealer:x.dealer,navigationLinks:t},l=new URLSearchParams({content:a,metadata:JSON.stringify(r)}),s=await fetch("https://automation.cloudcovehosting.com/webhook/pagecontext?"+l.toString(),{method:"GET",headers:{"Content-Type":"application/json",Origin:window.location.origin}});if(s.ok){var d=await s.text();if(d){var c=window.location.href,p=n,h=d;try{var g={url:c,hash:p,summary:h,timestamp:Date.now()};localStorage.setItem("bellaaiPageSummary",JSON.stringify(g))}catch(e){console.error("Error caching summary:",e)}return{summary:d,navigationLinks:t}}console.warn("Empty response from page context webhook")}else console.warn("Failed to generate page summary:",s.status,s.statusText)}else console.log("No text content found in main content")}else console.log("No main content found on page");return null}catch(e){return console.error("Error generating page summary:",e),null}var u,m,y}function B(){E&&clearTimeout(E),E=setTimeout(async()=>{S=await T()},1e3)}document.addEventListener("DOMContentLoaded",B);const L=window.location.href;function z(){[`Hi I am Bella, I am here to make your online shopping experience with ${x.dealer.name||"[DealerName]"} easy, but most of all save your time and money. Are you here to find a new vehicle or service your current one?`].forEach((t,e)=>{setTimeout(()=>{var e=document.createElement("div");e.className="chat-message bot",e.innerHTML=u(t),k.appendChild(e),k.scrollTop=k.scrollHeight,b()},1e3*e)})}new MutationObserver(()=>{var e=window.location.href;e!==L&&(L=e,B())}).observe(document,{subtree:!0,childList:!0});let N=null;function O(){N&&(s.appendChild(d),N.remove(),N=null),d.classList.remove("overtake-modal"),d.style.position="",d.style.left="",d.style.top="",d.style.transform="",d.style.zIndex="",d.style.width="",d.style.height="",d.style.maxHeight="",d.style.borderRadius="",d.style.boxShadow="",d.style.opacity="",d.style.background="",d.style.display="",d.style.flexDirection="",d.style.padding="",document.body.style.overflow="",D()}function P(){H(),r=!0,d.classList.add("open"),c.classList.add("hidden"),window.innerWidth<=600&&(document.body.style.overflow="hidden"),k.scrollTop=k.scrollHeight,v(),clearTimeout(i)}function D(){d.classList.remove("open"),c.classList.remove("hidden"),window.innerWidth<=600&&(document.body.style.overflow="")}function U(){let t=d.querySelector(".chat-input");if(t){t.innerHTML="";let a=document.createElement("button");a.type="button",a.title="Transfer to Phone",a.id="bellaai-voice-button",a.innerHTML=`
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path>
          </svg>
        `,a.style.display="flex",a.style.alignItems="center",a.style.justifyContent="center",a.style.background="linear-gradient(135deg, var(--chat--color-primary) 0%, var(--chat--color-secondary) 100%)",a.style.color="white",a.style.border="none",a.style.borderRadius="8px",a.style.padding="0 12px",a.style.cursor="pointer",a.style.fontWeight="500",a.style.fontSize="16px",a.style.height="40px",a.style.minWidth="40px",a.style.marginRight="8px",a.onclick=function(e){e.stopPropagation();e=document.getElementById("bellaai-voice-tooltip");if(e)e.remove();else{let o=document.createElement("div");o.id="bellaai-voice-tooltip",o.style.position="absolute",o.style.bottom="60px",o.style.left="0",o.style.background="white",o.style.boxShadow="0 2px 12px rgba(0,0,0,0.15)",o.style.borderRadius="8px",o.style.padding="10px",o.style.zIndex="9999",o.style.width="200px",o.style.boxSizing="border-box";e=document.createElement("style");e.textContent=`
          ...
          `,t.style.position="relative",t.appendChild(o),document.addEventListener("click",function e(t){o.contains(t.target)||t.target===a||(o.remove(),document.removeEventListener("click",e))}),document.getElementById("bellaai-start-call").onclick=async function(){var t=document.getElementById("bellaai-phone-input").value.trim();if(t)try{o.remove(),await _("phone",t);var e=document.createElement("div");e.className="chat-message bot",e.innerHTML=u("I'm calling your phone now. Please answer to continue our conversation."),k.appendChild(e),k.scrollTop=k.scrollHeight}catch(e){console.error("Error initiating phone call:",e);t=document.createElement("div");t.className="chat-message bot",t.innerHTML=u("I couldn't connect to your phone. Please check the number and try again."),k.appendChild(t),k.scrollTop=k.scrollHeight}}}};var e=document.createElement("textarea"),o=(e.placeholder="Type your message here...",e.rows=1,e.style.resize="none",e.style.flex="1",e.style.marginRight="8px",e.className=h.className,document.createElement("button"));o.type="submit",o.innerHTML='<svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>',o.style.display="flex",o.style.alignItems="center",o.style.justifyContent="center",o.style.background="linear-gradient(135deg, var(--chat--color-primary) 0%, var(--chat--color-secondary) 100%)",o.style.color="white",o.style.border="none",o.style.borderRadius="8px",o.style.padding="0 18px",o.style.cursor="pointer",o.style.fontWeight="500",o.style.fontSize="16px",o.style.height="40px",o.style.minWidth="40px",t.appendChild(a),t.appendChild(e),t.appendChild(o),o.addEventListener("click",A),e.addEventListener("keypress",function(e){"Enter"!==e.key||e.shiftKey||(e.preventDefault(),A())}),e.addEventListener("input",function(){this.style.height="auto",this.style.height=this.scrollHeight+"px"}),window.bellaaiTextarea=e}}function M(){try{var e,t=localStorage.getItem("bellaaiChatSession");return t?(e=JSON.parse(t)).messages&&Array.isArray(e.messages)?e.messages.map(e=>({role:"user"===e.type?"user":"assistant",content:"user"===e.type?e.content:e.content.replace(/<[^>]*>/g,""),timestamp:(new Date).toISOString()})):[]:[]}catch(e){return console.error("Failed to parse chat history",e),[]}}async function _(e,t){var o=M(),a=x.dealer;console.log("[BellaAI] Initiating voice call with dealer info:",a);let n="";try{var i=x.webhook.url.match(/\/webhook\/([^\/]+)/);i&&i[1]&&(n=(n=i[1]).replace(/\/chat$/,""))}catch(e){console.error("[BellaAI] Error extracting chatbot ID:",e)}let r={};if("phone_raw"===e)r={type:e,phone:t,chatbotId:n,dealerInfo:a};else if(r={type:e,chatHistory:o,dealerInfo:a,sessionId:y,chatbotId:n},"phone"===e){if(!t)throw new Error("Phone number required for phone call");r.phone=t}try{console.log("[BellaAI] Sending voice call payload:",JSON.stringify(r));var l,s,d=await fetch("https://automation.cloudcovehosting.com/webhook/voice-call",{method:"POST",headers:{"Content-Type":"application/json","Access-Control-Allow-Origin":"*","Access-Control-Allow-Methods":"POST, OPTIONS","Access-Control-Allow-Headers":"Content-Type"},body:JSON.stringify(r)});if(d.ok)return(s=d.headers.get("content-type"))&&s.includes("application/json")?d.json():{status:"success",message:await d.text(),callDetails:{phone:t,callId:"unknown",status:"connecting"}};throw l=await d.text(),new Error("Failed to initiate call: "+l)}catch(e){throw console.error("Error in initiateVoiceCall:",e),e}}(l=document.createElement("style")).textContent=`
      ...
      `,document.head.appendChild(e),document.addEventListener("keydown",function(e){"Escape"===e.key&&(N?O():d.classList.contains("open")&&D())})})();